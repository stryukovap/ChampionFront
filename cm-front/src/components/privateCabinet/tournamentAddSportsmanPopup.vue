...
<script>
    ...
    export default {
        ...
        props: ['tournamentKey', 'federationId', 'activeCategory', 'activeGenderCategory', 'activeWeightCategory'],
        data: function() {
            return {
                options : [],
                value: [],
        ...
        mounted() {
            if ('sportsmen' in this.tournament.categories[this.activeCategory][this.activeGenderCategory][this.activeWeightCategory]) {
                this.value = this.tournament.categories[this.activeCategory][this.activeGenderCategory][this.activeWeightCategory].sportsmen;
                ...
                .then(response => {
                    if (response.status === 200) {
                        response.data.data.forEach(item => {
                            if (item.date_of_birth.slice(0, 4) >= this.tournament.categories[this.activeCategory].ageFrom &&
                                item.date_of_birth.slice(0, 4) <= this.tournament.categories[this.activeCategory].ageTo &&
                                item.gender === this.activeGenderCategory[0].toUpperCase() &&
                                item.weight <= this.tournament.categories[this.activeCategory][this.activeGenderCategory][this.activeWeightCategory].weight) {
        ...
        methods: {
           async saveSportsmen() {
               if (this.tournament.categories[this.activeCategory][this.activeGenderCategory][this.activeWeightCategory].hasOwnProperty('sportsmen') === false) {
                   this.$set(this.tournament.categories[this.activeCategory][this.activeGenderCategory][this.activeWeightCategory], 'sportsmen', []);
               }
               ...
               console.log(this.amountOfParticipants - this.valueAmount + this.value.length);
               if ((this.amountOfParticipants - this.valueAmount + this.value.length) <=
